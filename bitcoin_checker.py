...
INSTALL:
    pip install pycryptodome        # fast AES (C-extension)
...
# ─────────────────────────────────────────────────────────────────────────────
# PURE PYTHON AES-256-CBC  (fallback without pip dependencies)
# ─────────────────────────────────────────────────────────────────────────────
...
# Build inverse S-box at module level
_INV_SBOX = [0] * 256
...
def _aes256_key_expand(key: bytes):
    """AES-256 key schedule → 15 round keys (each 16 bytes, column-major)."""
    w = [list(key[i:i+4]) for i in range(0, 32, 4)]  # 8 initial words
    for i in range(8, 60):
        t = w[i-1][:]
        if i % 8 == 0:
            t = [
                _SBOX[t[1]] ^ _RCON[i//8 - 1],
                _SBOX[t[2]],
                _SBOX[t[3]],
                _SBOX[t[0]],
            ]
...
def _aes256_decrypt_block(block: bytes, rk) -> bytes:
    state = list(block)
    ...
    for r in range(13, 0, -1):
        ...
        for c in range(4):
            ...
            out[c]    = _gmul(s0,0xe)^_gmul(s1,0xb)^_gmul(s2,0xd)^_gmul(s3,0x9)
            out[4+c]  = _gmul(s0,0x9)^_gmul(s1,0xe)^_gmul(s2,0xb)^_gmul(s3,0xd)
            out[8+c]  = _gmul(s0,0xd)^_gmul(s1,0x9)^_gmul(s2,0xe)^_gmul(s3,0xb)
            out[12+c] = _gmul(s0,0xb)^_gmul(s1,0xd)^_gmul(s2,0x9)^_gmul(s3,0xe)
        state = out
...
def aes256_cbc_decrypt_nopad(key: bytes, iv: bytes, ct: bytes) -> bytes:
    """AES-256-CBC decrypt, no padding removal."""
    if HAS_PYCRYPTO:
...
def parse_hash(line: str) -> dict:
    ...
    NOTE: mkHexLen = length of the HEX STRING (not byte count!)
    ...
    # BUG FIX: mkHexLen is the hex string length, NOT byte count.
    # No multiplication by 2 needed.
    ...
    if len(master_hex) < 64:
        raise ParseError(f"mkHex too short ({len(master_hex)} chars), min 64")
...
def derive_key(password: str, salt: bytes, iterations: int):
    """
    Round 1  : SHA512(password_bytes + salt)
    Round 2..N: SHA512(prev_hash)
    Returns  : key[0..31], iv[32..47]
    """
...
def valid_padding(dec: bytes) -> bool:
    """
    Bitcoin master key is always 32 bytes (AES-256).
    PKCS7 on 32 bytes → one full padding block → 16 × 0x10.

    Lenient validation (last >= 1 && last <= 16) → ~6.25% false positive rate.
    This strict validation → false positive ~1/(256^16) ≈ 0.
    """
    return len(dec) >= 16 and all(b == 0x10 for b in dec[-16:])


# ─────────────────────────────────────────────────────────────────────────────
# CHECK ONE PAIR (hash, password)
# ─────────────────────────────────────────────────────────────────────────────
def check_one(parsed: dict, password: str) -> bool:
    try:
        key, iv = derive_key(password, parsed["salt_bytes"], parsed["iterations"])
        dec     = aes256_cbc_decrypt_nopad(key, iv, parsed["master_bytes"][:32])
...
# ─────────────────────────────────────────────────────────────────────────────
# CPU WORKER  (runs in subprocess via Pool)
# ─────────────────────────────────────────────────────────────────────────────
def _worker(args):
...
# ─────────────────────────────────────────────────────────────────────────────
# Each GPU work-item processes one (hash, password) pair in parallel.
# SHA-512 iterative + AES-256-CBC + padding check — all inside the kernel.
# BUG FIX: SHA-512 single-block limit was 111 bytes (one 128-byte block).
# The combined input (password + salt) can exceed 111 bytes for long passwords.
# Fixed: kernel now handles two-block SHA-512 for inputs up to 239 bytes.
 ...
 * BUG FIX: original only supported 111 bytes (one block).
 * password (up to 63 bytes) + salt (typically 8 bytes) = up to 71 bytes.
 * Two-block support is implemented defensively for safety.
 */
void sha512(const uchar* data, int dlen, uchar out[64]) {
    ...
    /* length goes in last 16 bytes of the final block */
    ...
    /* Process block 2 only if needed */
    if(dlen >= 112){
        for(i=0;i<16;i++)
...
void aes256_expand(const uchar key[32], uchar rk[15][16]){
    ...
    for(i=0;i<15;i++)
        for(j=0;j<4;j++){
            rk[i][j*4+0]=w[i*4+j][0]; rk[i][j*4+1]=w[i*4+j][1];
            rk[i][j*4+2]=w[i*4+j][2]; rk[i][j*4+3]=w[i*4+j][3];
        }
}

void aes256_dec_block(uchar state[16], uchar rk[15][16]){
    int i,c; uchar tmp,ns[16];
    for(i=0;i<16;i++) state[i]^=rk[14][i];
    for(int r=13;r>=1;r--){
        ...
        for(i=0;i<16;i++) state[i]^=rk[r][i];
        for(c=0;c<4;c++){
            uchar s0=state[c],s1=state[4+c],s2=state[8+c],s3=state[12+c];
            ns[c]   =gmul(s0,0xe)^gmul(s1,0xb)^gmul(s2,0xd)^gmul(s3,0x9);
            ns[4+c] =gmul(s0,0x9)^gmul(s1,0xe)^gmul(s2,0xb)^gmul(s3,0xd);
            ns[8+c] =gmul(s0,0xd)^gmul(s1,0x9)^gmul(s2,0xe)^gmul(s3,0xb);
            ns[12+c]=gmul(s0,0xb)^gmul(s1,0xd)^gmul(s2,0x9)^gmul(s3,0xe);
        }
        for(i=0;i<16;i++) state[i]=ns[i];
    }
    tmp=state[13];state[13]=state[9];state[9]=state[5];state[5]=state[1];state[1]=tmp;
    tmp=state[10];state[10]=state[2];state[2]=tmp;
    tmp=state[14];state[14]=state[6];state[6]=tmp;
...
__kernel void btc_check(
    __global const uchar* passwords,   /* pw_stride bytes per password */
    __global const uchar* salt,
              const int   salt_len,
    __global const uchar* ct,          /* 32 bytes ciphertext */
              const int   iters,
    __global int*         results,     /* output: 1=match, 0=no */
    __global uchar*       found_pw,    /* output: matched password */
    __global int*         found_flag,  /* output: 1 if any match */
              const int   pw_stride
){
    ...
    /* KDF round 1: SHA512(password + salt) */
    ...
    /* Strict padding validation: last 16 bytes must all be 0x10 */
    int ok=1;
...
# ─────────────────────────────────────────────────────────────────────────────
class GPUEngine:
    PW_STRIDE = 64  # max password length
    ...
    def check_batch(self, parsed: dict, passwords: list):
        """Check a batch of passwords against one hash on GPU. Returns matched password or None."""
        mf = cl.mem_flags
        n  = len(passwords)
